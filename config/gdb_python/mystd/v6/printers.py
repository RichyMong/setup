...
class StdRbtreeIteratorPrinter:
    ...
    def to_string (self):
        ...
        return self.val.cast(nodetype).dereference()['_M_storage']
...
class StdMapPrinter:
    ...
    class _iter:
        ...
        def __next__(self):
            if self.count % 2 == 0:
                n = self.rbiter.__next__().cast(self.type)
                valtype = self.type.target().template_argument(0).pointer()
                n = n.dereference()['_M_storage']['_M_storage'].address.cast(valtype).dereference()
                self.pair = n
...
class StdCxx11StringPrinter:
    "Print a std::__cxx11::basic_string of some kind"
...
def build_libstdcxx_dictionary ():
    ...
    pretty_printers_dict[re.compile('^std::__cxx11::basic_string<.*>$')] = lambda val: StdCxx11StringPrinter(val)
